...
> For storage segment validation skills (iperf3, FIO, MinIO, SPEC CPU, etc.), see the `storage-skills` branch.  
> For CPU, memory, AMX, and wakeup micro-benchmark skills, see the `main` branch.
...
|---|---|---|
| [`rack-slurm`](.github/skills/rack-slurm/) | `/rack-slurm` | Install, configure, and operate Slurm on a DMR system |
| [`run-benchmark`](.github/skills/run-benchmark/) | `/run-benchmark [name\|all]` | Run CPU, memory, AMX, wakeup, and core-to-core benchmarks |
| [`emon-workload-sweep`](.github/skills/emon-workload-sweep/) | `/emon-workload-sweep` | Collect EMON traces while sweeping workload configs |
| [`create-skill`](.github/skills/create-skill/) | `/create-skill` | Write new SKILL.md files (meta skill) |

---

## rack-slurm

**Full skill:** [`.github/skills/rack-slurm/SKILL.md`](.github/skills/rack-slurm/SKILL.md)

Slurm Workload Manager (SchedMD) installed from GitHub source and configured for single-node DMR operation. The skill covers the complete workflow from a bare system to running job queues.
...
| Script | What it does |
|---|---|
| [`scripts/slurm-install.sh`](.github/skills/rack-slurm/scripts/slurm-install.sh) | Full install from source: packages → build → user → conf → systemd → start |
| [`scripts/gen-slurm-conf.sh`](.github/skills/rack-slurm/scripts/gen-slurm-conf.sh) | Auto-detects LAN IP / CPU count / RAM → writes `slurm.conf` + `cgroup.conf` |
| [`scripts/slurm-start.sh`](.github/skills/rack-slurm/scripts/slurm-start.sh) | Starts munge + slurmctld + slurmd; prints `sinfo` on success |
| [`scripts/slurm-test-jobs.sh`](.github/skills/rack-slurm/scripts/slurm-test-jobs.sh) | 7-test PASS/FAIL battery covering sbatch, srun, arrays, dependencies, scancel |
...
bash .github/skills/rack-slurm/scripts/slurm-install.sh

# Or if already installed, just start daemons
bash .github/skills/rack-slurm/scripts/slurm-start.sh

# Run the full job test battery
bash .github/skills/rack-slurm/scripts/slurm-test-jobs.sh
```
...
```
install and configure Slurm on this DMR system
run the full Slurm job test battery and show me which tests pass and fail
run all platform benchmarks and give me a summary
run the memory latency and bandwidth benchmark
collect EMON traces while running the AMX benchmark
write a new skill for [your workload]
```
...
```
.github/skills/
├── rack-slurm/               # Slurm install, configure, operate
│   ├── SKILL.md
│   └── scripts/
│       ├── slurm-install.sh
│       ├── gen-slurm-conf.sh
│       ├── slurm-start.sh
│       └── slurm-test-jobs.sh
├── run-benchmark/            # CPU / memory / AMX / wakeup benchmarks
│   ├── SKILL.md
│   └── references/
├── emon-workload-sweep/      # EMON collection + analysis
│   ├── SKILL.md
│   └── references/
└── create-skill/             # Meta skill — write new SKILL.md files
    └── SKILL.md
README.md                     # This file
```
